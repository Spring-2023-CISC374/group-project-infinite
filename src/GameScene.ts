...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    const coinCounterText = this.add.text(40, 10, `x ${coins}`, {
      fontSize: "24px",
      fill: "#000",
    });
    ...
    this.input.on("drop", (pointer, gameObject, dropZone) => {
      ...
      if (
        (gameObject === pinkfrost ||
          gameObject === bluefrost ||
          gameObject === yellowfrost) &&
        (dropZone === zone || dropZone === zone2)
      ) {
        updateCoinCounter();
      }
